...
# ISR Alnus xylem anomalies - LWBR GLMMs
library(here)
...
write.csv(
  tidy_best,
  file.path(output_dir, "Alnus_LWBR_current_model_coeffs.csv"),
  row.names = FALSE
)
...
write.csv(
  effect_table,
  file.path(output_dir, "Alnus_LWBR_current_model_OR.csv"),
  row.names = FALSE
)
...
# "late_CDD"     "carryoverCDD"
#"late_CDD" still lowest AICc

# R2
r.squaredGLMM(m_late_CDD)
r.squaredGLMM(m_carryoverCDD)

r2_supported <- lapply(supported_legacy, r.squaredGLMM)
r2_supported
...
###########plot of predicted LWBR occurrence with climate predictors
...
ggplot(all_preds_current, aes(x = orig_x, y = fit)) +
  ...
  labs(
    ...
    fill  = "Ring Type",
    color = "Ring Type",
    title = "Alnus LWBR - 95% CI"
  ) +
  scale_color_manual(values = c("LWBR" = "#E98960")) +
  scale_fill_manual(values  = c("LWBR" = "#E98960")) +
...
age_preds <- data.frame(
  RingAge = age_seq,
  fit   = plogis(pr_age$fit),
  lower = plogis(pr_age$fit - 1.96 * pr_age$se.fit),
...
ggplot(age_preds, aes(x = RingAge, y = fit)) +
  ...
  labs(
    ...
    fill  = "Ring Type",
    color = "Ring Type",
    title = "Alnus LWBR - ring age effect "
  ) +
  scale_color_manual(values = c("LWBR" = "#E98960")) +
  scale_fill_manual(values  = c("LWBR" = "#E98960")) +
  theme_classic(base_size = 12) +
