...
# ISR alnus xylem anomalies - LWBR Temp threshold
library(here)
...
for (sample in colnames(rings_alnus)[-1]) {
  for (i in 1:nrow(rings_alnus)) {
    
    year <- rings_alnus$year[i]
    value <- rings_alnus[i, sample]
    ...
    df_long_alnus <- rbind(df_long_alnus, data.frame(
      Year = year,
      SampleID = sample,
      raw_value = value_chr,
      present = pres,
...
df_long_alnus <- df_long_alnus %>% filter(present)


# merge latitude info
df_long_alnus <- df_long_alnus %>%
  left_join(lat_df_alnus, by = c("SampleID" = "ID"))
...
write.csv(
  T_range_df1,
  file.path(output_dir, "alnus_Tstar_bootstrap_range_10_90.csv"),
  row.names = FALSE
)
T_rangecentral
...
write.csv(
  T_range_df2,
  file.path(output_dir, "alnus_Tstar_bootstrap_range_2.5_97.5.csv"),
  row.names = FALSE
)
