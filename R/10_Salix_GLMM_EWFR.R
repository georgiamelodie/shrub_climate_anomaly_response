...
# ISR Salix xylem anomalies - EWFR GLMMs
...
#"GDD"  
### growing degree days dominant thermal mechanism
...
vars_current <- c(
  "tFEW","SampleID","lat_s","Age_s",
  # GDD
  "t2m_GDD_month5_s","t2m_GDD_month6_s","t2m_GDD_month7_s",
  # precip
...
# M1: Early growing season growing degree days (May-June)
m1_earlyGDD <- glmer(
  tFEW ~ lat_s + Age_s +
    t2m_GDD_month5_s + t2m_GDD_month6_s +
    (1 | SampleID),
...
# M2: Mid growing season warmth (July)
m2_julyGDD <- glmer(
  tFEW ~ lat_s + Age_s +
    t2m_GDD_month7_s +
    (1 | SampleID),
...
# M4: Combined early growing season GDD and precip (May-June)
m4_earlyGDDprecip <- glmer(
  tFEW ~ lat_s + Age_s +
    t2m_GDD_month5_s + t2m_GDD_month6_s +
    precip_month5_s + precip_month6_s +
    (1 | SampleID),
...
cand_set_current <- list(
  null            = m0_null,
  early_GDD       = m1_earlyGDD,
  july_GDD        = m2_julyGDD,
  early_precip    = m3_earlyPrecip,
  earlyGDD_precip = m4_earlyGDDprecip
)
...
# R2
r.squaredGLMM(m4_earlyGDDprecip)
...
vars_legacy <- c(
  ...
  "precip_month5_s","precip_month6_s",
  # preceding year 
  "t2m_GDD_month8_prev_s","t2m_GDD_month9_prev_s",
...
#P1: preceding year Aug GDD
m_prevAugGDD <- glmer(
  tFEW ~ lat_s + Age_s +
    t2m_GDD_month5_s + t2m_GDD_month6_s +
    t2m_GDD_month8_prev_s +
    (1 | SampleID),
  data = df_legacy,
  family = binomial,
  control = glmm_ctrl
...
m_prevAugGDD_prevSepPrecip <- glmer(
  tFEW ~ lat_s + Age_s +
    t2m_GDD_month5_s + t2m_GDD_month6_s +
    t2m_GDD_month8_prev_s + precip_month9_prev_s +
    (1 | SampleID),
  data = df_legacy,
  family = binomial,
  control = glmm_ctrl
...
legacy_set <- list(
  null                       = m0_null_legacy,
  current_earlyGDD           = m_current_earlyGDD,
  prevAugGDD                 = m_prevAugGDD,
  prevSepPrecip              = m_prevSepPrecip,
  prevAugGDD_prevSepPrecip   = m_prevAugGDD_prevSepPrecip
)
...
# "prevAugGDD_prevSepPrecip"

summary(m_prevAugGDD_prevSepPrecip)
...
write.csv(
  tidy_legacy,
  file.path(output_dir, "Salix_EWFR_prevyear_model_coeffs.csv"),
  row.names = FALSE
)
