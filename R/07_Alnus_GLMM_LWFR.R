...
# ISR Alnus xylem anomalies - LWFR GLMMs
...
vars_current <- c(
  ...
  #precip
  "precip_month6_s","precip_month7_s","precip_month8_s", "precip_month9_s"
)
...
#M5: precipitation constraint (Jul-Sep)
m5_precip <- glmer(
  tFLW ~ lat_s + Age_s +
    precip_month7_s + precip_month8_s +  precip_month9_s +
    (1 | SampleID),
  data = df_current,
  family = binomial,
  control = glmm_ctrl
...
#M6: Combined early growing season CDD and precip
m6_earlyCDDprecip <- glmer(
  tFLW ~ lat_s + Age_s +
    tmin_CDD_month6_s + tmin_CDD_month7_s +
    precip_month6_s + precip_month7_s +
    (1 | SampleID),
  data = df_current,
  family = binomial,
  control = glmm_ctrl
...
#M7: Combined late growing season CDD and precip
m7_lateCDDprecip <- glmer(
  tFLW ~ lat_s + Age_s +
    tmin_CDD_month8_s + tmin_CDD_month9_s +
    precip_month8_s + precip_month9_s +
    (1 | SampleID),
  data = df_current,
  family = binomial,
  control = glmm_ctrl
...
#M8: early and late growing season cold
m8_earlylateCDD <- glmer(
  tFLW ~ lat_s + Age_s +
    tmin_CDD_month5_s +
    tmin_CDD_month8_s + 
    (1 | SampleID),
...
cand_set_current <- list(
  ...
  precip           = m5_precip,
  earlyCDD_precip  = m6_earlyCDDprecip,
  lateCDD_precip   = m7_lateCDDprecip,
  earlylate_CDD    = m8_earlylateCDD
)
...
effect_table <- tidy_best %>%
  ...
  dplyr::mutate(
    Predictor = dplyr::recode(term,
                              ...
                              "tmin_CDD_month9_s" = "Sep CDD",
                              "precip_month8_s" = "Aug precip",
                              "precip_month9_s" = "Sep precip"
    ),
...
vars_legacy <- c(
  "tFLW","SampleID", "lat_s","Age_s", 
  #CDD
  "tmin_CDD_month5_s", "tmin_CDD_month8_s", "tmin_CDD_month9_s", 
  "tmin_CDD_month7_prev_s", "tmin_CDD_month8_prev_s", 
  "tmin_CDD_month9_prev_s",
  #precip
  "precip_month6_s","precip_month7_s","precip_month8_s", "precip_month9_s",
  "precip_month6_prev_s","precip_month7_prev_s",
...
#using significant current year model parameters
m_current <- glmer(
  tFLW ~ lat_s + Age_s +
    tmin_CDD_month8_s + tmin_CDD_month9_s +
    (1 | SampleID),
...
#add previous year Aug CDD 
m_prevCDD <- glmer(
  tFLW ~ lat_s + Age_s +
    tmin_CDD_month8_s + precip_month9_s +
...
#carry over CDD and precip
m_prevCDDprecip <- glmer(
  tFLW ~ lat_s + Age_s +
    tmin_CDD_month8_s + precip_month9_s +
...
legacy_set <- list(
  current_CDD      = m_current,
  prevCDD          = m_prevCDD,
  prevprecip       = m_prevprecip,
  prevCDDprecip    = m_prevCDDprecip,
  null             = m0_null_legacy
)
...
#LWFR curent year climate prediction plot
ggplot(all_preds_current, aes(x = orig_x, y = fit)) +
  geom_ribbon(
    aes(ymin = lower, ymax = upper, fill = "LWFR"),
    alpha = 0.25
  ) +
  geom_line(
    aes(color = "LWFR"),
